...
print("✅ Load model Facenet thành công!")

# Load SVM model và LabelEncoder
with open('svm_face_recognition.pkl', 'rb') as f:
    model_svm, out_encoder = pickle.load(f)
print("✅ Load SVM model và encoder thành công!")


# Hàm tiền xử lý ảnh và lấy embedding
...
# Khởi tạo camera (0 là webcam mặc định)
...
while True:
    ...
    for face in faces:
        ...
        embedding = get_embedding(model, face_region)

        # Dự đoán tên từ SVM model
        prediction = model_svm.predict([embedding])
        label = out_encoder.inverse_transform(prediction)
...
model_facenet = load_model("facenet_model/facenet_keras_2024.h5")
with open('svm_face_recognition.pkl', 'rb') as f:
    model_svm, out_encoder = pickle.load(f)
